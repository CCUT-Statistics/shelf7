...
define('PLUGIN_OFFICIAL_VER', '20240730');
define('PLUGIN_OFFICIAL_URL', DEBUG == 4 ? 'http://127.0.0.1/' : 'https://bbs.oddfox.cn/');
...
function plugin_init() {
    ...
    if(is_array($plugin_paths)) {
        ...
        if(!empty($plugins)){
            $total = count($plugins);
            $plugins_in = $plugins_en = $plugins_un = array();
            $plugins_in = arrlist_cond_orderby($plugins, array('installed'=>1, 'enable'=>1), array(), 1, $total);
            $plugins_en = arrlist_cond_orderby($plugins, array('installed'=>1, 'enable'=>0), array(), 1, $total);
            $plugins_un = arrlist_cond_orderby($plugins, array('installed'=>0, 'enable'=>0), array(), 1, $total);
            $plugins = $plugins_in + $plugins_en + $plugins_un;
        }
    }
}
...
/*
 * 清空缓存后要重新从官方插件中心获取数据，卡的蛋蛋疼；
 * 缓存官方插件数据到本地，超过6小时则从官方获取数据
*/
function fox_get_plugin_json(){
    global $time;
    $json_path = APP_PATH.'log/plugin-all-'.PLUGIN_OFFICIAL_VER.'.json';
    $filetime = xn_filemtime($json_path);
    //大于6小时则更新数据21600 = 6H
    if(($time - $filetime) > (3600 * 6)){
        $json_data = fox_write_plugin_json($json_path);
    }else{
        $json_data = file_get_contents_try($json_path);
        empty($json_data) AND $json_data = fox_write_plugin_json($json_path);
    }
    return $json_data;
}

//获取数据并写入本地文件
function fox_write_plugin_json($json_path){
    if($json_path){
        $json_url = PLUGIN_OFFICIAL_URL.'plugin-all-'.PLUGIN_OFFICIAL_VER.'.htm';
        $json_data = http_get($json_url);
        if(empty($json_data)) $json_data = array();
        $r = file_put_contents_try($json_path, $json_data);
        return $json_data;
    }
    return FALSE;
}
