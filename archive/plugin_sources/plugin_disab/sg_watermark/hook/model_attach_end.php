...
function sg_watermark($img, $ext) {
		...
		$sg_watermark = setting_get('sg_watermark');
		...
		if($sg_watermark['type'] == '1') {
			...
			$logo = imagecreatefromstring(file_get_contents('./plugin/sg_watermark/img/logo.png')); 
			...
			$temp = imagettfbbox($sg_watermark['size'],0,"./plugin/sg_watermark/font/$sg_watermark[font]",$sg_watermark['text']);
		...
		if($sg_watermark['type'] == '1') {
			...
			$font = "./plugin/sg_watermark/font/$sg_watermark[font]";
