...
if($method == 'GET') {
	...
	$logo = '../plugin/sg_watermark/img/logo.png?'.$time;
	include _include(APP_PATH.'plugin/sg_watermark/setting.htm');
} else {
	$action = param(3);
	if($action == 'logo') {
		...
		$path = '../plugin/sg_watermark/img/';
		$url = '../plugin/sg_watermark/img/logo.png';
