...
if($action == 'autolink') {
	if($method == 'GET') {
        ...
        $siteImg = param('siteImg');
        if(empty($siteTitle)) message(-1, '请填写网站名称');
        if(empty($siteUrl)) message(-1, '请填写网站主页链接');
        if(empty($siteDesc)) message(-1, '请填写网站简介');
        if(empty($siteImg)) message(-1, '请填写网站LOGO');

        if($gid == 1){
             db_create('autolink', array('uid'=>0, 'siteTitle'=>$siteTitle,'siteUrl'=>$siteUrl,'siteDesc'=>$siteDesc,'siteImg'=>$siteImg,'create_time'=>time(),'status'=>1));
             cache_delete('autolinks');
		...
		$link = db_find_one('autolink',array('uid'=>$uid));
		if(empty($link)){
		    db_create('autolink', array('uid'=>$uid, 'siteTitle'=>$siteTitle,'siteUrl'=>$siteUrl,'siteDesc'=>$siteDesc,'siteImg'=>$siteImg,'create_time'=>time(),'status'=>0));
		}else{
		    db_update('autolink',array('uid'=>$uid),array('siteTitle'=>$siteTitle,'siteUrl'=>$siteUrl,'siteDesc'=>$siteDesc,'siteImg'=>$siteImg,'create_time'=>time(),'status'=>0));
		}
