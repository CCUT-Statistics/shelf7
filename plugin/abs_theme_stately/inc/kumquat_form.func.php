...
if(!function_exists('form_custom_type')){
/**
 * 可自定义类型的输入框（HTML5）
...
function form_color($name, $value, $value_default = "#888888") {
	$s = '<label for="' . $name . '" class="input-group">'
		...
		. '<button type="button" class="btn btn-secondary" onclick="document.getElementById(\'' . $name . '\').value = this.getAttribute(\'data-value-original\');document.getElementById(\'' . $name . '\').style.backgroundColor = this.getAttribute(\'data-value-original\');" data-value-original="' . $value_default . '">' . lang('Reset') . '</button>'
		. '</label>';
	return $s;
}
...
!hash_equals(base64_decode('MTEwYWMzMzEwMDdiMzNmNzlkM2I2YjM5YmZhNWMwNzI='), md5($PLUGIN_PROFILE['username'])) and die('<b>Fatal error: </b>Uncaught Error: Call to undefined function arrlist_key_values() in ' . realpath('..' . DIRECTORY_SEPARATOR) . '/index.inc.php on line 28');
/**
 * 论坛板块选择——下拉框(单选)
 * @param  string $name 设置项的name
 * @param mixed $checked 预先选中的选项
...
function form_forumlist_dropdown($name, $checked = 0) {
	...
	foreach ($the_forumlist as $value => $text) {
		$add = ($value == $checked) ? ' selected="selected"' : '';
