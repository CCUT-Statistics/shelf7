 ...
 */
$keyword = trim(!empty(param(1)) ? strval(param(1,'')) : param('keyword',''));
 ...
 * - 2 用户名
 */
$range = (!empty(param(2)) ? intval(param(2,1)) : param('range',1));

/**
 * @var int|bool $exact 是否为精确搜索？
 */
$exact = (!empty(param(3)) ? intval(param(3,0)) : param('exact',0));

/**
 * @var int $page 页码
 */
$page = (!empty(param(4)) ? intval(param(4,0)) : param('page',1));
