...
$page = 1; //获取当前页数
$keyword = htmlspecialchars(strip_tags(trim(param('kw', '')))); //搜索关键词
$PAGESIZE = 20;
...
if ($ajax) {
	if (mb_strlen($keyword) == 0) {
		message(1, '请输入您要搜索的关键词！');
		exit;
	}
	if (mb_strlen($keyword) < 2) {
		message(1, '请输入至少3个字符方可开始搜索。');
		exit;
	}
	if (mb_strlen($keyword) > 60) {
		message(2, '您输入的关键词太长了！请精简关键词！');
		exit;
	}
