 ...
 * @param {int} timeout 自动隐藏时间（秒）；0 表示不隐藏。
 * @param {object} options 自定义Modal设置
 * - size: string 弹窗大小 sm|md|lg|xl
 * - scrollable: bool 允许在内容过多时纵向滚动？
 * - verticallyCentered: bool 垂直居中？
 * - noCloseOnOutsideClick: bool 点击弹窗外面不会关闭弹窗？
 * 
 * @returns {any} 一个Modal
 */
$.alert = function (subject, timeout, options) {
	...
	if (typeof(options.scrollable) != 'undefined' && options.verticallyCentered) {
		mymodal.verticallyCentered();
	}
	if (typeof(options.noCloseOnOutsideClick) != 'undefined' && options.noCloseOnOutsideClick) {
 ...
 * @returns {any} 一个Modal
 * 
 * ok_callback的第一个参数为和这个弹窗相关的数据（以对象的形式呈现），第二个及以后的参数可以用来传递参数
 */
$.confirm = function (subject, ok_callback, options) {
	...
	if (typeof(the_options.scrollable) != 'undefined' && the_options.scrollable) {
		mymodal.scrollableDialog();
	}
	if (typeof(the_options.scrollable) != 'undefined' && the_options.verticallyCentered) {
		mymodal.verticallyCentered();
	}
	if (typeof(the_options.noCloseOnOutsideClick) != 'undefined' && the_options.noCloseOnOutsideClick) {
		mymodal.staticBackdrop();
	}
 ...
 * @param {object} arg 其他参数，与请求一并发送
 * @param {object} options 自定义Modal设置
 * - backdrop: true 一定带背景，无需设置
 * - focus: true 一定聚焦到弹窗上，无需设置
 * - size: string 弹窗大小 sm|md|lg|xl
 * - scrollable: bool 允许在内容过多时纵向滚动？
 * - verticallyCentered: bool 垂直居中？
 * - noCloseOnOutsideClick: bool 点击弹窗外面不会关闭弹窗？
 ...
 * @returns 一个Modal
 */
$.ajax_modal = function (url, title, size, callback, arg, options) {
	...
	if (options.size != 'md') {
		jmodal.changeSize(options.size);
	}
	if (typeof(options.scrollable) != 'undefined' && options.scrollable) {
		jmodal.scrollableDialog();
	}
	if (typeof(options.scrollable) != 'undefined' && options.verticallyCentered) {
		jmodal.verticallyCentered();
	}
	if (typeof(options.noCloseOnOutsideClick) != 'undefined' && options.noCloseOnOutsideClick) {
		jmodal.staticBackdrop();
	}
	jmodal.show();
	// ajax 加载内容
