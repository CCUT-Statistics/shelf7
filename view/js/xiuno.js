	...
	$.ajax({
		...
		error: function(xhr, type) {
			if(retry > 1) {
				...
				var errmsg = xn.ajax_error_text(xhr, type);
				if((type != 'abort' && type != 'error') || xhr.status == 403 || xhr.status == 404) {
					return callback(-1000, errmsg);
				} else {
					return callback(-1001, errmsg);
				}
			}
		}
	});
...
xn.ajax_error_text = function(xhr, type) {
	var status = xhr && typeof xhr.status != 'undefined' ? xn.intval(xhr.status) : 0;
	var response_text = xhr && xhr.responseText ? $.trim(xhr.responseText + '') : '';
	var lower = response_text.toLowerCase();

	if(status == 413 || lower.indexOf('request entity too large') !== -1) {
		return '上传失败：请求体过大（HTTP 413）。请调大 Nginx 的 client_max_body_size，并同步调大 PHP 的 post_max_size 与 upload_max_filesize 后重试。';
	}

	if(status == 403) {
		return '请求被拒绝（HTTP 403）。请检查登录状态、权限和安全策略设置。';
	}

	if(status == 404) {
		return '请求地址不存在（HTTP 404）。请刷新页面后重试，若仍失败请检查路由或伪静态配置。';
	}
	...
	if(status === 0 && type == 'error') {
		return '网络异常或连接被中断，请检查网络与反向代理配置后重试。';
	}

	if(response_text.length > 240) {
		response_text = xn.substr(response_text, 0, 240) + '...';
	}

	if(response_text) {
		return '请求失败（HTTP ' + status + '，type=' + type + '）：' + response_text;
	}

	return '请求失败（HTTP ' + status + '，type=' + type + '）。';
}
